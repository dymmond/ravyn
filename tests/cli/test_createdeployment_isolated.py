...
def test_create_app_with_env_var(cli_tmp_dir: Path, safe_run_cmd):
    (o, e, ss) = _run_cmd(safe_run_cmd, "tests.cli.main:app", "ravyn createdeployment myproject")
    assert ss == 0
...
def test_create_app_without_env_var(cli_tmp_dir: Path, safe_run_cmd):
    (o, e, ss) = _run_cmd(
        safe_run_cmd, "tests.cli.main:app", "ravyn createdeployment myproject", is_app=False
    )
    assert ss == 0
