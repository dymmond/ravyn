...
def _run_asserts(base_dir: Path):
    ...
    # Deployment
    assert (base_dir / "myproject/deployment/docker/Dockerfile").is_file() is True
    assert (base_dir / "myproject/deployment/gunicorn/gunicorn_conf.py").is_file() is True
    assert (base_dir / "myproject/deployment/nginx/nginx.conf").is_file() is True
    assert (base_dir / "myproject/deployment/nginx/nginx.json-logging.conf").is_file() is True
    assert (base_dir / "myproject/deployment/supervisor/supervisord.conf").is_file() is True
