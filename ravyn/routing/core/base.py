...
class BaseResponseHandler:
    ...
    async def _get_response_data(
        ...
        if parameter_model.has_kwargs:
            if request is None:
                raise RuntimeError(
                    "Request is required when handler expects request-bound kwargs."
                )
        ...
        if isinstance(route.parent, BaseController):
            if fn_is_async:
                return await fn(route.parent, **parsed_kwargs)
            return fn(route.parent, **parsed_kwargs)

        if fn_is_async:
            return await fn(**parsed_kwargs)
    ...
    def _get_json_response_handler(
        ...
        resolved_cookies = self.get_cookies(cookies) if cookies else []
        resolved_headers = {
            **(self.get_headers(headers) if headers else {}),
            **self.allow_header,
        }
    ...
    def _get_response_handler(
        ...
        static_cookies = self.get_cookies(cookies) if cookies else []
        resolved_headers = {
            **(self.get_headers(headers) if headers else {}),
            **self.allow_header,
        }
        ...
        async def response_content(data: Response, **kwargs: dict[str, Any]) -> LilyaResponse:
            _cookies = self.get_cookies(data.cookies, cookies) if data.cookies else static_cookies
            for cookie in _cookies:
    ...
    def _get_default_handler(
        ...
        async def response_content(data: Any, **kwargs: dict[str, Any]) -> LilyaResponse:
            ...
            if isinstance(data, LilyaResponse):
                if (
                    not resolved_cookies
                    and self.background is None
                    and not self.__is_status_overridden__
                ):
                    return data

                if self.__is_status_overridden__:
                    data.status_code = self.status_code
                elif getattr(data, "status_code", None) is None:
                    data.status_code = self.status_code

                if self.background is not None and getattr(data, "background", None) is None:
